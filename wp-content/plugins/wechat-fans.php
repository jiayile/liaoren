...
　　名称：<input name="huoduan_wechatfans[wechat_name]" type="text" value="<?php echo isset($wechatfans['wechat_name'])?$wechatfans['wechat_name']:'聊仁云屋';?>" /> <span>微信公众号平台→公众号设置→名称，例如：聊仁云屋</span>

</p> 
<p>
<strong>微信公众号：</strong><br />
　微信号：<input name="huoduan_wechatfans[wechat_account]" type="text" value="<?php echo isset($wechatfans['wechat_account'])?$wechatfans['wechat_account']:'huoduan';?>" /> <span>微信公众号平台→公众号设置→微信号，例如：506873775</span>

</p> 
<p>
<strong>回复以下关键词获取验证码：</strong><br />
　关键词：<input name="huoduan_wechatfans[wechat_keyword]" type="text" value="<?php echo isset($wechatfans['wechat_keyword'])?$wechatfans['wechat_keyword']:'微信验证码';?>" /> 例如：<span style="color:#F00;">验证码</span>，访客回复这个关键词就可以获取到验证码
...
图片地址：<input name="huoduan_wechatfans[wechat_qrimg]" style="width:400px;" type="text" value="<?php echo isset($wechatfans['wechat_qrimg'])?$wechatfans['wechat_qrimg']:'https://www.liaoren.vip/static/images/wechat150.gif';?>" /> <button class='custom_media_upload button'>上传</button> 填写您的微信公众号的二维码图片地址，建议150X150像素
</p>
<p>
...
　　密钥：<input name="huoduan_wechatfans[wechat_key]" type="text" value="<?php echo isset($wechatfans['wechat_key'])?$wechatfans['wechat_key']:md5('yunziyuan.com.cn'.time().rand(10000,99999));?>" /> 用于加密Cookie，默认是自动生成，一般无需修改，如果修改，所有访客需要重新输入验证码才能查看隐藏内容
</p>
...
如有疑问可以点击<a href="https://www.liaoren.vip/" target="_blank">这里</a>咨询作者，聊仁云屋官方网站：<a href="https://www.liaoren.vip" target="_blank">https://www.liaoren.vip</a><br />
</p>
</div>   
<?php   
...
function huoduan_wechat_fans($content){
	...
	{
	    $wechatfans = get_option('huoduan_wechatfans');
		$cv = md5($wechatfans['wechat_key'].$cookie_name.'yunziyuan.com.cn');
		$vtips='';
